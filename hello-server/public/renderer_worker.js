    ...
    const camera = new Camera(videoElem, {
        onFrame: async () => {
            ...
            if (state.getMaskRawData() && (state.useBackgroundImage || state.useBackgroundColor)) {
                ...
                // WebAssembly.compileStreaming(fetch("hello-world-simple.wasm")).then((mod) =>
                //     worker.postMessage({task:'loadModule', mod:mod})
                // );
                worker.postMessage({
                    graph:state.graph,
                    task:'runWithMask',
                    imgPointer:state.imgPointer,
                    maskPointer:state.maskPointer,
                    imgSize:state.imgSize,
                });
                console.log('data posted');
                state.graph = await new Promise((resolve, reject) => {
